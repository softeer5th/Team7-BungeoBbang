...
public class MemberOnlyChecker {
     ...
     * - 메서드 인자 중 `Accessor`를 구현한 객체가 있는 경우, `isMember()`을 호출하여 관리자 여부를 확인합니다.
     * - 관리자가 아닐 경우, `AuthException`을 발생시킵니다.
     * </p>
     *
     * @param joinPoint 현재 실행 중인 메서드의 실행 정보 (AOP)
     * @throws AuthException 회원이 아닐 경우 `INVALID_AUTHORITY (403)` 예외 발생
     */
    @Before("@annotation(com.bungeobbang.backend.auth.member.MemberOnly)")
    public void check(final JoinPoint joinPoint) {
        Arrays.stream(joinPoint.getArgs())  // 메서드 인자 확인
                ...
                .filter(Accessor::isMember) // `isAdmin()`을 통해 관리자 여부 확인
                .findFirst()
